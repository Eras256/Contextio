...
export interface LegalContextDoc {
  ...
  consentRequirements: { id: string; description: string; required: boolean; scope: string[] }[];
  disputeChannels: { type: string; provider: string; venue: string; governingLaw: string; language: string }[];
  settlement: { networks: string[]; assets: string[] };
  publishedAt: string;
}

/** Renders the live, published LCP document — `doc` is always the real fetched
 * document (fetched server-side in page.tsx); this component never invents data. */
export default function LegalContextView({ doc }: { doc: LegalContextDoc }) {
  const t = useT();
