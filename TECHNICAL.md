...
| **SEP-38** | Real indicative FX quotes (`GET /public/anchor/sep38`) from the same reference anchor's `ANCHOR_QUOTE_SERVER` — e.g. live XLM/USD, XLM/CAD. USDC pricing is listed by the anchor but currently 502s on its own backend (verified via direct curl, not our bug); XLM works end-to-end today. | ✅ Live (XLM) |
| **SEP-31** | Real institutional-send discovery (`GET /public/anchor/sep31`) against the anchor's `DIRECT_PAYMENT_SERVER`. Its `receive` asset list is currently empty on this reference anchor, so no settlement is possible yet — protocol integration is real, there's just nothing to send until a licensed anchor relationship exists (see Milestone 2). | ✅ Live (discovery only) |
...
- **Anchor Platform-compatible SEP-31/38 client** as the integration layer for a **licensed SEP-24/31 anchor** — SEP-31 is the institutional/corridor-appropriate standard for payroll disbursement (SEP-24 alone, live before this, is the retail-interactive flow); SEP-38 supplies a firm FX quote before settlement, replacing the volatility buffer estimate.
  - **Live on testnet (2026-08-04):** `AnchorClient.getSep38Prices`/`getSep31Info` (`apps/api/src/integrations/anchor.ts`) hit the *same* SDF reference anchor already used for SEP-24 — its `stellar.toml` already publishes `ANCHOR_QUOTE_SERVER` (SEP-38) and `DIRECT_PAYMENT_SERVER` (SEP-31) alongside SEP-24, no separate anchor needed for this layer. Public proof: `GET /api/v1/public/anchor/sep38` (live indicative XLM/USD + XLM/CAD pricing — verified; the anchor's own USDC price entry currently 502s on its backend, confirmed via direct curl, not our integration) and `GET /api/v1/public/anchor/sep31` (live discovery — this anchor's `receive` asset list is currently empty, so nothing can actually settle through it yet).
  - **Still gated on a business step, not code:** actually *sending* a SEP-31 payment needs a licensed LATAM off-ramp partner (e.g. a PIX/SPEI/PSE-capable provider like BlindPay or Koywe) with assets configured to receive — that relationship doesn't exist yet, and no amount of additional integration code changes that. The protocol-level client is real and proven; the settlement corridor is not.
- **Stellar Disbursement Platform (SDP)** for the bulk-payout engine itself — committed in the original CV Labs Integration track application ("Blend v2 + DeFindex + SDP") but not yet built. SDP is a real multi-service deployment (dashboard + core API + transaction-submission service, Postgres, an SMS/email provider, its own distribution + SEP-10 accounts) built on top of Anchor Platform — not a client library, a system to stand up. Sequenced after Anchor Platform for that reason: deploying SDP without Anchor Platform underneath it first would mean re-deploying it once that layer lands.
